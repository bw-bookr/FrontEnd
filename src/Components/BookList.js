...
export default class BookList extends React.Component {
 ...
 Axios
.get("https://bookr-app-backend.herokuapp.com/api/book-collection", requestOptions)
...
.then(res => {
  ...
  render() {
    ...
    //   </Container>
    // ));
    console.log("What data before return stmt ", this.state.bookList);
    return (
      <Container >
        <Row>
          {console.log("What data  ", this.state.bookList)}
       { this.state.bookList.map(book => (<Col lg="4" md="6" className="mb-3" key={book.id}><Book {...book} /></Col>))}
       </Row>
      </Container>
    );
